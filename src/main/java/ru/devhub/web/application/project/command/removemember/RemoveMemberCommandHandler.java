...
@Service
public class RemoveMemberCommandHandler implements CommandHandler<RemoveMemberCommand, ProjectMember> {

    private final ProjectRepository projectRepository;
    private final ProjectMemberRepository projectMemberRepository;

    public RemoveMemberCommandHandler(ProjectRepository projectRepository,
    ...
    @Override
    @Transactional
    public ProjectMember handle(RemoveMemberCommand command) {
        Project project = projectRepository.findById(command.projectId())
                .orElseThrow(() -> new ProjectNotFoundException(command.projectId()));
        Project updated = project.removeMember(command.memberId(), command.requestingUserId());
